...
class DLList{
   ...
   public:
    ...
    void remove(T val){
        ...
        if(cur == tail && cur->val == val){ //se l'elemento cercato è la coda
            ...
            return;
                ...
                std::cout << "Element with value " << val << " not found" << "\n\n";
                return;
        }
    ...
    friend std::ostream& operator<<(std::ostream& out, const DLList<T> &DLList){
         if(!DLList.isEmpty()){
            out << "List head = " << DLList.head << ", List tail = " << DLList.tail 
                << std::endl; 
